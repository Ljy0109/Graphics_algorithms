...
bool Material::hasEmission() {  //  检查材质是否具有发射光
...
Vector3f Material::sample(const Vector3f &wi, const Vector3f &N){
    switch(m_type){ // 两种材质进行相同的操作
        case Microfacet:
        case DIFFUSE:
        {   // 在单位球面上随机取一个点，然后变换到世界坐标系下
...
Vector3f Material::eval(const Vector3f &wi, const Vector3f &wo, const Vector3f &N){
    switch(m_type){
        case DIFFUSE:
        {
            ...
            // Lambert's Model 朗伯模型定义的 BRDF 函数值是一个常数 rho/pi
            // rho是albedo，也就是反照率
