...
void rst::rasterizer::draw_line(Eigen::Vector3f begin, Eigen::Vector3f end)
{
    ...
    if(dy1<=dx1)
    {
        ...
        Eigen::Vector3i point = Eigen::Vector3i(x, y, 1.0f);
        ...
        for(int j=minY;j<maxY;j++)
        {
            ...
            if(IsUseSurperSampling)
            {
                ...
                {   // 使用重心坐标插值颜色、法向量、纹理坐标、
                    ...
                    set_pixel(Eigen::Vector3i(i,j,1),pixel_color*IsInTriangleCount/4.0f);
                }
            }
            else
            {
                if(insideTriangle(i+0.5f,j+0.5f,t.v))
                {
                    ...
                    if(z_interpolated<depth_buf[Index])
                    {
                        ...
                        set_pixel(Vector3i(i,j,1), pixel_color);
...
void rst::rasterizer::set_pixel(const Eigen::Vector3i& point, const Eigen::Vector3f& color)
{
...
{   // 将函数对象std::function<Eigen::Vector3f(vertex_shader_payload)> 实例化
    // 也就是说 Eigen::Vector3f vertex_shader(vertex_shader_payload)
