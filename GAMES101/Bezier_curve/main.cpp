...
std::vector<cv::Point2f> control_points; // 曲线控制点
...
{   // 贝塞尔曲线反走样
    ...
    for (float i = 0; i < 1; i+=0.001f)
    {
        ...
        for(int k=0;k<=1;k++)
        {
            for(int j=0;j<=1;j++)
            {
                float distance=get_distance(cv::Point2f(minx+k,miny+j),point);
                std::cout<<"distance = " << distance <<std::endl;
                float sqrt_2 = std::pow(2, 0.5f);
                float color = std::max(std::min(255.0f*(sqrt_2-distance),255.0f), 0.0f);
